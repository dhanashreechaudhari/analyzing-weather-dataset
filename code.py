...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
    instance = pd.DataFrame( df[ (df[col1]>val1) & (df[col2]==val2) ] )
    return instance
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    This function accepts a dataframe, 2 columns(feature) and aggregated funcion(agg)     which returns the Pivot table with different aggregated value of the feature with     an index of the month.
    ...
    aggregated_value = df.pivot_table( index=df[date_col].dt.month, values=[agg_col],     aggfunc={agg_col:aggregate[agg]})
...
def group_values(df,col1,agg1):
    ...
    This function accepts a dataframe, 1 column(feature) and aggregated function(agg1)    which groupby the dataframe based on the column.
...
def convert(df,celsius):
    ...
    """
    converted_temp = df[celsius].apply(lambda x : x * 9/5 + 32)
...
print("Categorical:", categorical(weather).shape)
print("Numerical:", numerical(weather).shape)
print('_'*80)
...
#You can check it by calling thr function clear with respective parameters.
print("Cloudy Weather:", clear(weather, 'Weather', 'Cloudy'))
print('_'*80)
...
print("Speed and visibility:", wind_speed_35_vis_25.shape)
print('_'*80)
...
print("Aggregated values in a month:")
...
print("Group values on Mean Weather column:")
mean_weather = group_values(weather, 'Weather', 'mean')
print(mean_weather)

# Feel free to try on diffrent aggregated functions like max, min, sum, len
print("Group values on Max, Min, Sum, Len")
print( group_values(weather, 'Weather', 'mean') )
print('_'*80)
...
print( convert(weather, 'Temp (C)').head(3) )
